...
def prog(name_out,  *pics):    #visual_mode=False,
    text_color=None
    data=[]
    for path in pics:
        #plt.figure()
...
#for x in range(0,len(points)):
 #   plt.plot(X,Y)
        ...
        for point in points:
            if maxl[0]>point[0] and len(text_color)/2+len(text_color)//8 >point[1]> len(text_color)/2-len(text_color)//8:
                maxl=point
            if maxp[0]<point[0] and len(text_color)/2+len(text_color)//8>point[1]> len(text_color)/2-len(text_color)//8:
                maxp=point
...
if __name__ == "__main__":
    ...
    prog(args.output_file,  *args.image_files)  #args.visual_mode,